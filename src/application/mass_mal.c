...
u16 MAL_Init(u8 lun) {
  ...
  switch (lun) {
    case 0:
     if((Status = SD_Init()) != SD_OK) showCriticalStatus(" SD init error [code:%d]\n  Only RESET possible at now", Status);
     scr_puts("\nRead CSD/CID MSD registers..");
     if((Status = SD_GetCardInfo(&SDCardInfo)) != SD_OK) showCriticalStatus(" SD get card info\n  error [code:%d]\n  Only RESET possible at now", Status);
     scr_printf("\nSD size:%d\nblock:%d type:%d", SDCardInfo.CardCapacity, SDCardInfo.CardBlockSize, SDCardInfo.CardType);
     scr_puts("\nSD select..");
     if((Status = SD_SelectDeselect((u32)(SDCardInfo.RCA << 16))) != SD_OK) showCriticalStatus(" SD select card\n  error [code:%d]\n  Only RESET possible at now", Status);
     if((Status = SD_SetDeviceMode(SD_POLLING_MODE)) != SD_OK) showCriticalStatus(" SD set device mode\n  error [code:%d]\n  Only RESET possible at now", Status);

     u32 DeviceSizeMul, NumberOfBlocks = 0;
     DeviceSizeMul = (SDCardInfo.SD_csd.DeviceSizeMul + 2);
     if(SDCardInfo.CardType == SDIO_HIGH_CAPACITY_SD_CARD)  Mass_Block_Count[0] = (SDCardInfo.SD_csd.DeviceSize + 1) * 1024;
     else {
